...
CREATE TABLE IF NOT EXISTS analyzer_rules (
  ...
  created_at    INTEGER NOT NULL
	...
	// v9: repair prompt_templates rows that stored literal backslash-n instead of real newlines
	if ver == 8 {
		tx, err := s.db.BeginTx(ctx, nil)
		if err != nil {
			return err
		...
		_, err = tx.ExecContext(ctx, `UPDATE prompt_templates SET prompt_text = REPLACE(prompt_text, '\n', char(10)) WHERE prompt_text LIKE '%\n%';`)
