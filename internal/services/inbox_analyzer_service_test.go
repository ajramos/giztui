...
func TestMergeCategories(t *testing.T) {
	...
	// Display order — action first, then name: archive:"Newsletters" < label:"Follow up".
	assert.Equal(t, "Newsletters", merged[0].Name)
...
// The plan must stay in display order after every merge: the panel renders the action
// verb before the name ("Archive · …"), so sort by action first, then by name.
func TestMergeCategoriesSortedForDisplay(t *testing.T) {
	existing := []ActionPlanCategory{
	...
	// archive:Newsletters < label:Billing < trash:alerts
	assert.Equal(t, []string{"Newsletters", "Billing", "alerts"}, names)
}
