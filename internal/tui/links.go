...
func (a *App) openLinkPicker() {
	...
	if a.logger != nil {
		...
		a.logger.Printf("LINKS DEBUG: cached='%s', cursor='%s', match=%t", messageID, cursorID, messageID == cursorID)
		
		// If they don't match, sync the cached state
		if messageID != cursorID && cursorID != "" {
			a.logger.Printf("LINKS SYNC: Cached ID is stale, updating from cursor position")
			messageID = cursorID
	...
	go func() {
		...
		a.QueueUpdateDraw(func() {
			...
			bgColor := a.GetComponentColors("links").Background.Color()
			...
			footer.SetTextColor(a.GetComponentColors("links").Text.Color()) // Standardized footer color
